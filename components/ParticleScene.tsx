...
const ParticleScene: React.FC<ParticleSceneProps> = ({ params }) => {
  const mountRef = useRef<HTMLDivElement>(null);
  const rendererRef = useRef<THREE.WebGLRenderer | null>(null);
  const particlesRef = useRef<THREE.Points | null>(null);
  // Fix: useRef expected 1 argument, but got 0. Initializing with undefined to satisfy the requirement.
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < params.count; i++) {
      const i3 = i * 3;
      // Initial positioning (random sphere)
    ...
    // Material with custom soft circular shader logic (simulated with points)
        ...
        for (let i = 0; i < params.count; i++) {
          ...
          switch (params.mode) {
            ...
            case ParticleMode.GALAXY:
              ...
              const spiral = 1.0 / (dist + 0.1) * speed;
              ...
              // Slight wave
              positionsArr[i3 + 1] += Math.sin(elapsedTime * speed + dist * 2) * 0.002;
              break;
  ...
  }, [params.count]); // Re-init if count changes drastically, otherwise we'd need more complex buffer resizing

  // Update dynamic properties without full re-init
  useEffect(() => {
    if (particlesRef.current) {
